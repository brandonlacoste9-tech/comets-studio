...
These components need refactoring to work without @v0-sdk/react:

1. **components/chat/chat-messages.tsx**
   - Currently imports `StreamingMessage` from `@v0-sdk/react`
   - Needs custom streaming component for DeepSeek responses
   - Used for rendering streaming AI responses

2. **components/shared-components.tsx**
   - Imports types and components from `@v0-sdk/react`
   - Includes: `ThinkingSection`, `CodeProject`, `CodeProjectPartProps`
   - Wrapper component adapts AI Elements to @v0-sdk/react format

3. **components/message-renderer.tsx**
   - Imports `Message`, `MessageBinaryFormat` from `@v0-sdk/react`
   - Used for rendering completed messages
